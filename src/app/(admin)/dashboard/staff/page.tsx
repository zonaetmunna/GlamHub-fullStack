...
export default function StaffPage() {
  ...
  const handleFilterChange = (filterType: string, value: string) => {
    switch (filterType) {
      case "role":
        ...
        break;
      case "status":
        if (value === "active") setActiveFilter(true);
        else if (value === "inactive") setActiveFilter(false);
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      case "ADMIN":
        return <FaShieldAlt className="w-4 h-4 text-red-400" />;
      case "STAFF":
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {staffData?.pagination?.totalCount || 0}
              </p>
            </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {staffData?.data?.filter((staff) => staff.isActive).length || 0}
              </p>
            </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {staffData?.data?.filter((staff) => staff.role === "STAFF")
                  .length || 0}
              </p>
            </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {staffData?.data?.filter((staff) => staff.role === "ADMIN")
                  .length || 0}
              </p>
            </div>
            ...
            <tbody>
              {staffData?.data?.map((staff) => (
                <tr key={staff.id} className="border-t border-white/10">
                  ...
                  <td className="p-4">
                    <div
                      className={`inline-flex items-center space-x-2 px-2 py-1 text-xs rounded-full ${getRoleColor(
                        staff.role
                      )}`}
                    >
                  ...
                  <td className="p-4">
                    <div>
                      <p className="text-white text-sm">{staff.email}</p>
                      {staff.phone && (
          ...
          <div className="p-4 border-t border-white/10">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-400">
                Showing{" "}
                {(staffData.pagination.page - 1) * staffData.pagination.limit +
                  1}{" "}
                to{" "}
                {Math.min(
                  staffData.pagination.page * staffData.pagination.limit,
                  staffData.pagination.totalCount
                )}{" "}
                of {staffData.pagination.totalCount} staff members
              </div>
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => setCurrentPage(currentPage - 1)}
                  disabled={!staffData.pagination.hasPreviousPage}
                ...
                <span className="text-white">
                  Page {staffData.pagination.page} of{" "}
                  {staffData.pagination.totalPages}
                </span>
                <button
                  onClick={() => setCurrentPage(currentPage + 1)}
                  disabled={!staffData.pagination.hasNextPage}
                  className="px-3 py-1 bg-white/10 text-white rounded-lg hover:bg-white/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Next
                </button>
